...
[高仿朋友圈照片查看器(二) 用PhotoView实现手势缩放图片](/2016/05/26/Android-PhotoViewer-2/)

[高仿朋友圈照片查看器（三）ViewPager支持阻尼回弹](/2016/05/26/Android-PhotoViewer-3/)

## 前言

最近公司的项目需要模仿朋友圈照片查看的效果，故在此记录下。Gif有点卡，实际效果很流畅：）
...
- 以九宫格方式展示1-9张缩略图
- 点击缩略图查看大图，平滑的过渡效果，移动&缩放
- 大图支持手势缩放，Pinch，拖动，双击放大，单击关闭
- 查看大图的Viewpager or Gallery边界的阻尼回弹效果


## 思考

九宫格展示缩略图的部分略去不讲，本节主要讲解点击缩略图之后如何平滑过渡到大图。九宫格上的缩略图都是以centerCrop方式显示，而点击之后的大图都是以fitCenter显示，仅仅用动画无法实现该效果。

观察朋友圈的过渡动效，包含如下属性

1. 背景从透明变为黑色，对应属性 *alpha*
2. 图片从缩略图位置过渡到大图位置，对应属性 *left*,*top*
3. 图片从缩略图大小过渡到大图大小，对应属性 *width*,*height*

重写ImageView，在onDraw中通过裁切canvas实现从centerCrop到fitCenter,通过PropertyValuesHolder控制对应的5个属性的变化
